...
from data.utils.inference_utils import apiqa, grading_model_name
from data.utils.io_utils import jdump, jload, question_hash, tload
...
def _process_question(question: str, msc_prompt: str, system_prompt: str):
    ...
    while result is None:
        completion, _ = apiqa(
            user_prompt, grading_model_name(), system_prompt, json_format=False
        )
...
def do_domain_classification():
    ...
    questions = [
        ...
        if question_hash(question) not in existing_qhash
    ]
...
def _process_example_grading(qhash: str, grading_prompt: str, response_dir: str):
    ...
    if example["solution"] is None:
        ...
        while grade is None:
            completion, _ = apiqa(
                user_prompt, grading_model_name(), grading_prompt, json_format=False
            )
            if completion.split("\n")[-1] in ["Yes", "No"]:
...
def do_grading(response_dir: str = "Qwen_Qwen2_5_32B_Instruct"):
    true_qhashes = [
        question_hash(q) for q in load_dataset("qfq/train")["train"]["question"]
    ]
    jsons = glob(
        f"results/difficulty_classification/{response_dir}/grading_input/*.json"
    )
    ...
    graded_qhashes = glob(
        f"results/difficulty_classification/{response_dir}/grading_output/*.json"
    )
