...
# ================= 2. 启动评测（跳过 SFT）=================
# 直接拉取 simplescaling/s1.1-32B，跑原版 Budget Forcing（强塞 "Wait"）推理机制。
# 统一入口见 scripts/run_eval.sh（内置 watchdog + 飞书通知 + 双卡数据并行）。
echo ">>> [$(date)] 评测开始，跳过 SFT 训练..."
start_time=$(date +%s)

# 评测失败也要继续往下走去停机，因此这里临时关闭 set -e
set +e
...
if [ "${eval_rc}" -eq 0 ]; then
    ...
    echo ">>> [$(date)] 评测失败（exit=${eval_rc}）！耗时: ${cost_time} 分钟，请查看日志。"
fi
...
# 【情况 A：runpod（当前环境）】
# 必须用 runpodctl 停止 pod 才会真正停止计费；容器内 `shutdown -h now` 在 runpod 上不生效。
